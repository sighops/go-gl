...
func main() {
  ...
  //quadPos := mgl32.Vec3{0.0, 0.0, 0.0}
  ...
  model := mgl32.Ident4()
  //model = mgl32.Translate3D(quadPos.X(),quadPos.Y(),quadPos.Z()).Mul4(model)
  ...
  for !window.ShouldClose() {
    ...
    if glfw.GetTime() - last_time >= 1.0/max_fps {
      ...
      projection := mgl32.Perspective(mgl32.DegToRad(45.0), float32(wWidth)/float32(wHeight), 0.1, 10.0)
      projectionUniform := gl.GetUniformLocation(program, gl.Str("projection\x00"))
      gl.UniformMatrix4fv(projectionUniform, 1, false, &projection[0])
    }
  }
