...
// 
// All of the code on this page has been written a gazillion u_times by a zillion different people. There is nothing new here. I guess that's why
...
// Larger numbers can give rounder looking corners, and can even take away a little aliasing, someu_times. However, larger numbers are obviously less 
...
// obvious to some, but the entire concept of raymarching is based on evaluating isosurface functions, so someu_times it helps to plug in some real-world 
...
// interesting, but this is just a simple template. The "u_time" term moves the bumps around a little, which gives the object a bit of movement and gives
...
float sinusoidBumps(in vec3 p){
    ...
    return sin(p.x*16.+0.57)*cos(p.y*16.+2.17)*sin(p.z*16.-1.31) + 0.5*sin(p.x*32.+0.07)*cos(p.y*32.+2.11)*sin(p.z*32.-1.23);
}
    
// This is the raymarched scene that gets called multiple u_times from the raymarching procedure, which means adding detail can slow things right down.
...
float rayMarching( vec3 origin, vec3 dir, float start, float end ) {
  ...
  for ( int i = 0; i < maxIterations; i++ ) {
    ...
    sceneDist = scene( origin + dir * rayDepth ); // Distance from the point along the ray to the nearest surface point in the scene.
        ...
        // accurate, but can slow your program down - drastically, at u_times. Larger numbers can speed things up, but at the cost of aesthetics.
    ...
    if (( sceneDist < stopThreshold ) || (rayDepth >= end)) {
        ...
        // likely, just the far-clipping-plane. Either way, it's u_time to return the maximum distance.
...
void main(void) {
    ...
    // from [0 to u_resolution.x] along the x-axis, and [0 to u_resolution.y] along the y-axis... If you wish to get technical, I hear they
    // range from [0.5 to u_resolution.x-0.5] and [0.5 to u_resolution.y-0.5] inclusive, but you know what I mean. :)
    //
    ...
    // aspect ratio ( screen_width/screen_height ), which in this case is (u_resolution.x/u_resolution.y).
    //
  ...
  // Yeah, I agree, the above is confusing. I wrote this shit, and if I read it for the first u_time, it'd confuse me too... but don't worry, I once knew 
  ...
  // write one myself at some stage.
    ...
    // lookAt - We'll someu_times have to change this to suit the camera movements. Again, movement instructions need to be made before the camera setup.
  ...
  vec3 lp = vec3(1.5*sin(u_time*0.5), 0.75+0.25*cos(u_time*0.5), -1.0);
  ...
  // Light falloff (attenuation), which depends on how far the surface point is from the light. Most of the u_time, I guess the falloff rate should be 
  // mixtures of inverse distance powers, but in real life, it's far more complicated than that. Either way, most of the u_time you should simply 
  ...
  // with regard to specular light calculations is getting the vector directions wrong, and I've made the mistake more than a few u_times. So, if you
